...
@implementation UIBubbleTableView{
    bool shouldLoad; // flag used to diable multiple loads to the same page;
}
...
// add limit number of data into section starting from start;
-(void)addBubbleData{
    // Cleaning up
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    float y = offset.y + inset.top;
    
    float reload_distance = 5;
    if(y < reload_distance && shouldLoad)
    {
        //Call the Method to load More Data...
        NSLog(@"in reload distance");
        shouldLoad = false;
        [self.containerDelegate bubbleTableViewReachedTop]; // should start loading new data;
    }
}
...
-(void)scrollViewWillBeginDragging:(UIScrollView *)scrollView{
    [self.containerDelegate bubbleTableViewWillScroll];
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    NSLog(@"calling number of rows section  ");
...
- (float)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return MAX(data.insets.top + data.view.frame.size.height + data.insets.bottom, self.showAvatars ? 52 : 0) + 20;
}
